...
class MHSA(nn.Module):
    def __init__(self, head, channels, R):
        super(MHSA, self).__init__()
        C = channels
        self.ln0 = nn.GroupNorm(1, C)
        self.relu = nn.ReLU()
        self.u_conv = nn.Conv1d(C, C//R, kernel_size=1, bias=False, padding=0)
        self.conv1 = nn.Conv1d(C//R, C//R, kernel_size=3, bias=False, padding=2, dilation=2)
        self.ln1 = nn.GroupNorm(1, C//R)
        self.conv2 = nn.Conv1d(C//R, C//R, kernel_size=3, bias=False, padding=3, dilation=3)
        self.ln2 = nn.GroupNorm(1, C//R)
        self.conv3 = nn.Conv1d(C//R, C//R, kernel_size=3, bias=False, padding=4, dilation=4)
        self.ln3 = nn.GroupNorm(1, C//R)
        self.d_conv = nn.Conv1d(C//R, C, kernel_size=1, bias=False, padding=0)
        

    def forward(self, input):
        if type(input) is not list:
            raise ValueError('The input should be list of features')
        x = input
        ...
        for x_ in x:    
            if len(x_.shape) != 3:
                k = True
                shape_x = x_.shape
                x_ = x_.view([x_.shape[0],-1,x_.shape[-1]])
            x_ = self.ln0(x_)
            xs.append(x_)
        x = torch.cat(xs, dim=-1)

        x0 = self.u_conv(x)
        x1 = self.ln1(self.relu(self.conv1(x0)))
        x2 = self.ln2(self.relu(self.conv2(x1)))
        x3 = self.ln3(self.relu(self.conv3(x2)))
        x = self.d_conv(x1+x2+x3) + x

        if k:
            x = x.view([shape_x[0], shape_x[1], shape_x[2], x.shape[-1]])
...
class ResNetSE(nn.Module):
    def __init__(self, block=SEBasicBlock, layers=[3, 4, 6, 3], num_filters=[16, 32, 64, 128], nOut=256, encoder_type='SAP', n_mels=80, log_input=True, **kwargs):
        ...
        self.mhsa = MHSA(head=4, channels=1280, R=32)
        self.mse_loss = nn.MSELoss()

        ## If you want analsys, Do self.forward = self.entire_wave2emb or self.entire_wave2emb_div
        # self.forward = self.entire_wave2emb_div ## for use FlopCountAnalysis (Comment out def forward)         
    ...
    def feats2emb(self, feats): # may not use
        ...
        late_feat = self.mhsa(late_feats)            
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                for feat in feats:
                    late_feat = self._before_pooling(feat)
                    late_feats.append(late_feat)
                fixed_feats = self.mhsa(late_feats)
                # fixed_feats = torch.cat(late_feats, dim=-1)
                emb = self._before_penultimate(fixed_feats) 
    ...
    def _compute_loss(self, x, t_feats, sub_frame, max_frame=False):
        late_feats = []
        feats = self.wave2feats(x, sub_frame=sub_frame, remain_hop=0, max_frame=max_frame)
        for feat in feats:
            late_feat = self._before_pooling(feat)
            late_feats.append(late_feat)
        fixed_feats = self.mhsa(late_feats)
        # fixed_feats = torch.cat(late_feats, dim=-1)
        # print('t_feats size :', t_feats.size())
        # print('fixed_feats size :', fixed_feats.size())
        mse_loss = self.mse_loss(fixed_feats, t_feats)
        
        emb = self._before_penultimate(fixed_feats)
        return emb, mse_loss

    # For comparing counter part, num_hop should be larger than one
    # def entire_wave2emb_div(self, wave, sub_frame=50, max_frame=300, num_hop=5, hop_feats=False, print_options=False): 
    def entire_wave2emb_div(self, wave, sub_frame=50, max_frame=200, num_hop=False, hop_feats=False, print_options=False): 
        import math
        sr = 160
        if num_hop:
            if num_hop < 1:
                raise Exception(" 'num_hop' should be larger than one or set to False")
            elif num_hop > max_frame//sub_frame:
                raise Exception(" Too large 'num_hop'")
...
###
    # For comparing counter part, num_hop should be larger than one
    def entire_wave2emb_div_AMI(self, wave, sub_frame=50, max_frame=300, num_hop=False, hop_feats=False, print_options=False): 
        import math
        sr = 160
        if num_hop:
            if num_hop < 1:
        ...
        shift_frame = max_frame-(num_hop*sub_frame)
        shift_len = shift_frame*sr
        num_for_loop = int(entire_frame/shift_frame)
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                for i, _ in enumerate(range(num_for_loop)):
                    # print(i)
                    if end_len >= wave.shape[-1]:
                        return embs     

                    if i == num_for_loop-1 and end_len > wave.shape[-1]: 
                        # last loop
                        remain_len = end_len - wave.shape[-1]
                        _hop = math.ceil(remain_len/sub_frame/sr)
                        
                        if _hop > 0:
                            end_len = wave.shape[-1]
                            start_len = end_len -_hop*sub_frame*sr
                            sub_wave_frame = _hop*sub_frame
...
###
    ...
    # def entire_wave2emb(self, wave, max_frame=100, hop_frame=50, print_options=False): 
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                for i, _ in enumerate(range(num_for_loop)):
                    ...
                    fixed_feats = self.mhsa([feat])
                    emb = self._before_penultimate(feat) 
                    embs.append(emb)
        return embs
    
    def forward(self, x, hop_num, max_frame, hop_frame):

        import math
        sr = 160
        entire_frame = math.ceil(x.shape[-1]/sr)
        if entire_frame < int(101 * max_frame):
            x = self.wave2emb(x)
        else:
            x = self.entire_wave2emb_div_AMI(wave=x, max_frame=int(100 * max_frame), sub_frame=50, num_hop=hop_num)
            # x = self.entire_wave2emb_AMI(wave=x, max_frame=int(100 * max_frame), hop_frame=int(100 * hop_frame))
