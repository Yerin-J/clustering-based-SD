...
def score(ref_turns, sys_turns, uem, step=0.010, nats=False, jer_min_ref_dur=0.0,
    ...
    # ref = Annotation()
    # for file_id, turn_list in file_to_ref_turns.items():
    #     min_onset_ref = min(turn.onset for turn in turn_list)
    #     max_offset_ref = max(turn.offset for turn in turn_list)
    #     for turn in turn_list:
    #         ref[Segment(turn.onset, turn.offset)] = turn.speaker_id

    # hyp = Annotation()
    # for file_id, turn_list in file_to_sys_turns.items():
    #     min_onset_hyp = min(turn.onset for turn in turn_list)
    #     max_offset_hyp = max(turn.offset for turn in turn_list)
    #     for turn in turn_list:
    #         hyp[Segment(turn.onset, turn.offset)] = turn.speaker_id

    # min_onset = min(min_onset_ref, min_onset_hyp)
    # max_offset = max(max_offset_ref, max_offset_hyp)
    # uem2 = Segment(min_onset, max_offset)
    # print(min_onset, max_offset)

    # CalcDER = DiarizationErrorRate(collar=2*0.25, skip_overlap=True)
    # CalcJER = JaccardErrorRate(collar=0.0, skip_overlap=False)
    # der = CalcDER(ref, hyp, detailed=True, uem=uem2)
    # jer = CalcJER(ref, hyp, detailed=True, uem=uem2)
    # pprint(der)
    # pprint(jer)
    # ## =========================================================== ##
