...
import utils.wavread as wavread
from utils.io_utils import read_aud
...
def get_uttq2utts(scplist, postfix_list=[]):
  if isinstance(scplist, str):
    scplist = [scplist]
  for scpfile in scplist:
    assert os.path.isfile(scpfile), '%s does not exist!' % scpfile
...
def make_ark(scpfile, endian='<', ark_out=''):
  if isinstance(scpfile, list):
    for _scpfile in scpfile:
      assert os.path.isfile(_scpfile), '%s does not exist!' % _scpfile
    pass
    for _scpfile in scpfile:
      make_ark(_scpfile, endian)

  elif isinstance(scpfile, str):
    ext = scpfile.rsplit('.', 1)[-1]
    assert os.path.isfile(scpfile), '%s does not exist!' % scpfile
    assert ext == 'scp', 'file extension must be "scp"'

    arkfile = scpfile.rsplit('.', 1)[0] + '.ark'
    with open(arkfile, 'wb') as f:
      for line in open(scpfile):
        uttID, arkp = line.rstrip().split(' ')
        data = read_kaldi_binary(arkp)
        write_kaldi_binary(f, data, key=uttID)
...
def make_scp(arkfile, endian='<', scp_out=''):
  if isinstance(arkfile, list):
    ...
    if scp_out != '':
      ...
      for _scp in scplist:
        assert _scp != scp_out, '"scp_out" must not exist!'
      os.makedirs(os.path.dirname(scp_out), exist_ok=True)
...
def make_reco2dur(wpathlist, fs, reco2dur, level=2):
  os.makedirs(os.path.dirname(reco2dur), exist_ok=True)
  with open(reco2dur, 'wt') as f:
    for wpath in tqdm(wpathlist):
      wpath = wpath.rstrip()
      ext = wpath.rsplit('.', 1)[-1]
      _, uttID = get_uttinfo(wpath, level)
      if ext == 'wav':
        nsamp = wavread.nsamples(wpath)
      else:
