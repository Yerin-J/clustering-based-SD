...
def twoGMMcalib_lin(s, niters=20):
    ...
    for _ in range(niters):
        lls = np.log(weights)-0.5*np.log(var) - 0.5*(s[:,None]-means)**2/var
...
def kaldi_ivector_plda_scoring_dense(kaldi_plda, x, target_energy=0.1, pca_dim=None):
    ...
    """
    plda_mu, plda_tr, plda_psi = kaldi_plda # (128,) (128, 128) (128,)

    [energy, PCA] = spl.eigh(np.cov(x.T, bias=True)) # (128,) (128, 128)
    if pca_dim is None:
    ...
    # PCA = np.eye(x.shape[1], dtype=x.dtype) #!@#
    ...
    # B = plda_tr_inv_pca.dot(plda_psi).dot(plda_tr_inv_pca.T)
    acvar, wccn = spl.eigh(B, W)
    x = np.dot(x-plda_mu, PCA).dot(wccn)
    x *= np.sqrt(x.shape[1] / np.dot(x**2, 1./(acvar+1.)))[:, None] # kaldi style length-norm
