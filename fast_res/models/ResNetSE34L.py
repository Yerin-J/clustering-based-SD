...
class ResNetSE(nn.Module):
    ...
    def entire_wave2emb_div(self, wave, sub_frame, max_frame, num_hop=False, hop_feats=False, print_options=False): 
        import math
        sr = 160
        if num_hop:
            if num_hop < 1:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                for i, _ in enumerate(range(num_for_loop)):
                    ...
                    if i == num_for_loop-1: 
                        # last loop
                        start_len = wave.shape[-1] -max_frame*sr +num_remain_hop*sub_frame*sr
                    ...
                    feats = self.wave2feats(sub_wave, sub_frame=sub_frame, remain_hop=num_remain_hop, max_frame=max_frame)
                    for feat in feats:
                        late_feat = self._before_pooling(feat)
                        late_feats.append(late_feat)
                    fixed_feats = self.mhsa(late_feats)
